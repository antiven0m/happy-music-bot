...
async def is_audio_requester(ctx):
    ...
    if permissions.administrator or state.is_requester(ctx.author):
        ...
        raise commands.CommandError(
            "You need to be the song requester to do that.  ☜(`o´)")
...
class Music(commands.Cog):
    ...
    async def leave(self, ctx):
        ...
        if client and client.channel:
            ...
            raise commands.CommandError("Not in a voice channel. ＼(｀0´)／")
    ...
    async def skip(self, ctx):
        ...
        if ctx.channel.permissions_for(
            ...
            await ctx.send(
                f"{ctx.author.mention} votes to skip ({len(state.skip_votes)}/{required_votes} votes) ( ﾟｰﾟ)"
            )
        else:
            raise commands.CommandError("Sorry, vote skipping is disabled. ( •̀ᴗ•́ )و")
    ...
    def _queue_text(self, queue):
        if len(queue) > 0:
            ...
            message += [
                f"  {index+1}. **{song.title}** (Played by **{song.requested_by.name}**) (´∀｀)"
                for (index, song) in enumerate(queue)
            ]  # add individual songs
            return "\n".join(message)
        else:
            return "The play queue is empty. (´ π`)"
    ...
    async def jumpqueue(self, ctx, song: int, new_index: int):
        ...
        if 1 <= song <= len(state.playlist) and 1 <= new_index:
            ...
            raise commands.CommandError("You must use a valid number. (¬▂¬)")
    ...
    async def play(self, ctx, *, url):
        ...
        if client and client.channel:
            ...
            message = await ctx.send(
                "Added to queue. (⌐▨_▨)", embed=video.get_embed())
            await self._add_reaction_controls(message)
        else:
            if ctx.author.voice is not None and ctx.author.voice.channel is not None:
                ...
                raise commands.CommandError(
                    "You need to be in a voice channel to do that. ヾ( ･`⌓´･)ﾉﾞ")
            ...
            if message.guild and message.guild.voice_client:
                ...
                if permissions.administrator or (
                    ...
                    required_votes = math.ceil(
                        self.config["vote_skip_ratio"] * users_in_channel)
                    await channel.send(
                        f"{user.mention} voted to skip ({len(state.skip_votes)}/{required_votes} votes) (⚆ᗝ⚆)"
                    )
